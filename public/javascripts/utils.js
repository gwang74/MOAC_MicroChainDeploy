...
const scsUri = nconf.get("scsUri");
const privatekey = nconf.get("privatekey");
const ercAddr = nconf.get('ercAddr');
...
function waitBlockForContractInMicroChain(microChain, transactionHash) {
...
function waitBlockForTransactionInMicroChain(microChain, transactionHash) {
...
function deployMicroChainWithAddr() {
    ...
    var microChain
    if (ercAddr) {
        microChain = path.resolve(__dirname, "../contract/") + "/" + 'ChainBaseAST.sol';
    } else {
        microChain = path.resolve(__dirname, "../contract/") + "/" + 'ChainBaseASM.sol'
    }
    var input = {
        '': fs.readFileSync(microChain, 'utf8'),
