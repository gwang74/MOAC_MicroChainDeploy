...
contract ReserveToken is StandardToken, SafeMath {
  ...
  function destroy(address account, uint amount) public {
    require(msg.sender == minter, "Not minter");
    require(amount < balances[account], "Insufficient funds");
...
contract EtherDelta is SafeMath {
  ...
  function changeFeeMake(uint feeMake_) public {
    require(msg.sender == admin, "Not admin");
    require(feeMake > feeMake_, "feeMake too low");
    feeMake = feeMake_;
  }

  function changeFeeTake(uint feeTake_) public {
    require(msg.sender == admin, "Not admin");
    require(feeTake > feeTake_ && feeRebate < feeTake_, "feeTake too low");
    feeTake = feeTake_;
  }

  function changeFeeRebate(uint feeRebate_) public {
    require(msg.sender == admin, "Not admin");
    require(feeRebate < feeRebate_ && feeTake > feeRebate_, "feeRebate too low");
    feeRebate = feeRebate_;
  }
  ...
  function withdraw(uint amount) public {
    require(amount < tokens[0][msg.sender], "Insufficient funds");
  ...
  function withdrawToken(address token, uint amount) public {
    require(token!=0);
    require(amount < tokens[token][msg.sender], "Insufficient funds");
  ...
  /**
   * 币币兑换
   * 商家换取token，需支付手续费，金银商户可获取回扣
   * 普通用户换取token，需支付手续费
   *
   * @param tokenGet 商家换取的token
   * @param amountGet 商家换取token的总数量
   * @param tokenGive 商家兑换的token
   * @param amountGive 商家兑换token的总数量
   * @param user 商家
   * @param amount 兑换数量
   */
